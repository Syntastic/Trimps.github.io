...
function load(saveString, autoLoad, fromPf) {
    ...
    if (
        ...
        compareVersion(
            oldStringVersion,
            game.global.stringVersion.split('.'),
            true,
        )
    ) {
        message(
    ...
    for (var a in game) {
        ...
        if (a == 'options' && savegame.options) {
            for (var itemO in savegame.options.menu) {
                ...
                if (itemO == 'mapAtZone') {
                    game.options.menu.mapAtZone.setZone =
                        savegame.options.menu.mapAtZone.setZone;
                    if (savegame.options.menu.mapAtZone.setZoneU2)
                        game.options.menu.mapAtZone.setZoneU2 =
                            savegame.options.menu.mapAtZone.setZoneU2;
                }
            }
            if (
                typeof savegame.options.menu.GeneticistassistTarget !==
                    'undefined' &&
        ...
        for (var b in topGame) {
            ...
            if (typeof midSave !== 'object') midGame = midSave;
                ...
                for (var c in midGame) {
                    ...
                    if (typeof botSave === 'undefined' || botSave === null)
                        continue;
                    if (a == 'heirlooms') {
    ...
    if (oldVersion < 1.1) {
        if (game.global.world >= 25) {
            for (
                var mys = 0;
                mys < Math.floor((game.global.world - 20) / 5);
                mys++
            ) {
    ...
    if (oldVersion < 2.213) {
        for (var item in game.options.menu) {
            game.options.menu[item].enabled = game.options.menu[item].enabled
                ? 1
                : 0;
        }
    }
    ...
    if (oldVersion < 2.7) {
        ...
        if (game.global.totalHeliumEarned > 0) {
            ...
            for (var item in game.portal) {
                ...
                if (
                    typeof portUpgrade.level === 'undefined' ||
                    portUpgrade.level <= 0
                )
                    continue;
    ...
    if (oldVersion < 4.7) {
        ...
        if (game.global.spiresCompleted >= 3)
            game.portal.Cunning.locked = false;
        if (game.global.spiresCompleted >= 4)
            game.portal.Curious.locked = false;
        addNewSetting('smallPerks');
        if (game.options.menu.masteryTab.lockUnless())
    ...
    if (oldStringVersion == null) {
        ...
        if (game.achievements.oneOffs.finished.length < 48) {
            for (
                var x = game.achievements.oneOffs.finished.length;
                x < 48;
                x++
            ) {
    ...
    if (compareVersion([5, 0, 0], oldStringVersion)) {
        ...
        if (savegame.talents) {
            game.talents.heirloom.purchased =
                savegame.talents.turkimp2 &&
                savegame.talents.turkimp2.purchased;
            game.talents.turkimp2.purchased =
                savegame.talents.turkimp3 &&
                savegame.talents.turkimp3.purchased;
            game.talents.heirloom2.purchased =
                savegame.talents.turkimp4 &&
                savegame.talents.turkimp4.purchased;
    ...
    if (compareVersion([5, 1, 0], oldStringVersion)) {
        ...
        if (game.global.CoreEquipped.name)
            game.global.CoreEquipped.icon = 'adjust';
        for (var hc = 0; hc < game.global.heirloomsCarried.length; hc++) {
            var loom = game.global.heirloomsCarried[hc];
            loom.id = 3 + hc;
            var type = loom.type;
        ...
        if (game.buildings.Microchip.owned > 5)
            game.buildings.Microchip.owned = 5;
    }
    //End compatibility
    //Test server only
    ...
    if (
        game.global.mapGridArray.length > 0 &&
        game.global.currentMapId !== ''
    ) {
        drawGrid(true);
    ...
    if (game.options.menu.darkTheme.enabled != 1)
    ...
    if (
        ...
        typeof game.challenges[game.global.challengeActive].onLoad !==
            'undefined'
    )
        game.challenges[game.global.challengeActive].onLoad();
    ...
    if (game.global.totalVoidMaps > 0 && !game.global.mapsActive)
        addVoidAlert();
    ...
    if (send) {
        ...
        if (document.getElementsByClassName('pauseMsg').length == 0) {
            var text =
                ...
                (game.options.menu.hotkeys.enabled
...
function portalClicked(noUniChange) {
    ...
    document.getElementById('totalHeliumEarned').innerHTML = prettify(
        totalEarned,
    );
    ...
    if (game.global.canRespecPerks) {
        ...
        document.getElementById('respecPortalBtn').style.display =
            'inline-block';
    }
...
function displayChallenges() {
    ...
    for (var what in game.challenges) {
        ...
        if (challengeSquaredMode)
                ...
                getChallengeSquaredButtonColor(what);
...
function selectChallenge(what) {
    ...
    if (challengeSquaredMode) {
        ...
        desc +=
            ' Your highest Zone reached for this Challenge<sup>' +
            number +
            '</sup> is Z' +
            highestZone +
            ', earning you ' +
        ...
        case 5: {
            return reward
                ? 'permanently increase all ' +
                      heliumOrRadon() +
                      " found by 0.5% to the power of your current Zone number. You'll also start with 1000% player efficiency and 50 Barns, Sheds, and Forges"
                : 1500;
        }
    }
}
        ...
        if (!restart) {
            ...
            if (
                ...
                (game.global.world > 30 ||
                    (game.global.world == 30 &&
                        game.global.lastClearedCell >= 29))
            )
                unlockJob('Meteorologist');
        }
    }
    game.global.challengeActive = '';
    if (challenge.fireAbandon && typeof challenge.abandon !== 'undefined')
        challenge.abandon();
    cancelPortal();
...
function getSquaredDescriptionInRun(hideDesc) {
    ...
    var dif =
        getIndividualSquaredReward(
            game.global.challengeActive,
            game.global.world,
        ) - getIndividualSquaredReward(game.global.challengeActive);
    ...
    if (highest > 0) {
        if (dif >= 1) {
            description +=
                ", <span class='greenText'>your previous highest for this challenge was Zone " +
                highest +
                '. If you ' +
                portalText +
                "now, you'll add " +
            ...
            if (game.global.highestRadonLevelCleared >= 64) {
                var c2 =
                    game.global.universe == 1 ? rewards[0] + dif : rewards[0];
                var c3 =
                    game.global.universe == 1 ? rewards[1] : rewards[1] + dif;
                description +=
            ...
            description +=
                ", <span class='redText'>your best for this challenge is Zone " +
                highest +
                '. If you ' +
                portalText +
        ...
        if (dif <= 0) {
            ...
            description +=
                ...
                "now, you'll add " +
                prettify(dif) +
                '% to your Challenge<sup>' +
                number +
                '</sup> bonus, bringing the total to ' +
                prettify(dif + reward) +
                '%.';
            if (game.global.highestRadonLevelCleared >= 64) {
                var c2 =
...
function viewPortalUpgrades() {
    ...
    if (c2Btn) {
        if (
            game.global.highestLevelCleared >= 64 &&
            game.global.runningChallengeSquared
        ) {
            c2Btn.style.display = 'inline-block';
            document.getElementById(
                'challengeSquaredBonusAmtView',
            ).innerHTML = prettify(game.global.totalSquaredReward);
    ...
    document.getElementById('portalHelium').innerHTML =
        '<span id="portalHeliumOwned">' +
        prettify(parseInt(totalAvailable, 10)) +
        '</span> ' +
        resName +
        ' Left Over';
...
function swapToCurrentChallenge(updateOnly) {
    ...
    if (lookingAtCurrentChallenge) {
        ...
        document.getElementById(
            'viewChallengeText',
        ).innerHTML = getCurrentChallengePane();
        updatePortalChallengeAbandonButton();
...
function getCurrentChallengePane() {
    ...
    if (game.global.challengeActive == 'Scientist') {
        ...
        description = description.replace(
            '*',
            getScientistInfo(sciLevel, true),
        );
    }
...
function isPerkUnlocked(what, usePortalUniverse) {
    ...
    var universe = usePortalUniverse ? portalUniverse : game.global.universe;
    if (universe == 2) {
        if (typeof perk.radLocked !== 'undefined' && !perk.radLocked)
            return true;
        return false;
    } else {
...
function displayPortalUpgrades(fromTab) {
    ...
    for (var what in game.portal) {
        ...
        if (
            portalUniverse == 2 &&
            (typeof portUpgrade.radLevel === 'undefined' ||
                portUpgrade.radLocked)
        )
            continue;
        if (
            portalUniverse == 1 &&
        ...
        if (game.options.menu.smallPerks.enabled)
            htmlClass +=
                game.options.menu.smallPerks.enabled == 1
                    ? ' smallPerk'
                    : ' tinyPerk';
    ...
    if (
        ...
        manageStacks(
            ...
            stacks +
                ' stack' +
                needAnS(stacks) +
                ' of Equality are active, reducing the Attack of Trimps and Bad Guys by ' +
                prettify((1 - Math.pow(0.9, stacks)) * 100) +
                '%.',
            false,
        );
    } else {
...
function updatePerkColor(what) {
    ...
    if (game.global.removingPerks) {
        ...
        if (usingScreenReader) {
            var affordElem = document.getElementById(
                'screenReaderPerkAfford' + what,
            );
            if (affordElem) {
        ...
        var canSpend =
            game.resources.helium.respecMax -
            game.resources.helium.totalSpentTemp;
...
var offlineProgress = {
        ...
        for (var x = 0; x < 4; x++) {
            ...
            if (x == 4 && price > frags) {
                ...
                this.mapTextElem.innerHTML =
                    ...
                    prettify(frags) +
                    ' Fragments.';
            this.mapBtns[x].style.display =
                price > frags ? 'none' : 'inline-block';
    ...
    updateBar: function(current) {
        ...
        if (game.global.mapsActive) {
            ...
            this.inMapDescriptionElem.innerHTML =
                'Sitting in the Map Chamber (lame)';
        if (current == 0) {
        ...
        if (mapStatus == 1) {
            ...
            selectMap(
                game.global.mapsOwnedArray[
                    game.global.mapsOwnedArray.length - 1
                ].id,
            );
            runMap();
            this.mapBtnsElem.style.display = 'none';
    ...
    formatTime: function(seconds) {
        ...
        if (minutes < 60)
            return (
                ...
                needAnS(minutes) +
                (seconds > 0
                    ? ' and ' + seconds + ' second' + needAnS(seconds)
                    : '')
            );
        ...
        if (hours <= 24)
            return (
                ...
                needAnS(hours) +
                (minutes > 0
                    ? ' and ' + minutes + ' minute' + needAnS(minutes)
                    : '')
            );
...
function trustworthyTrimps(noTip, forceTime) {
    ...
    for (var x = 0; x < compatible.length; x++) {
        ...
        amt +=
            amt * getPerkLevel('Motivation') * game.portal.Motivation.modifier;
            ...
            if (timeAtLastOnline < loops && chunkPercent < 100) {
                ...
                for (var z = timeAtLastOnline; z < loops; z++) {
                    ...
                    if (loops == 12)
                        amt +=
                            toAlter *
                            game.jobs.Magmamancer.getBonusPercent(false, z);
                }
            }
            if (remaining) {
                ...
                if (medLevel)
                    amt +=
                        toAlter *
                        (1 + game.portal.Meditation.getBonusPercent() * 0.01);
                if (loops == 12)
...
function presetTab(tabNum) {
    swapClass(
        'tab',
        'tabNotSelected',
        document.getElementById('presetTabSave'),
    );
...
function resetPresets() {
    ...
    for (var x = 1; x <= 3; x++) {
        ...
        document.getElementById(
            'presetTab' + x + 'Text',
        ).innerHTML = preset.Name ? preset.Name : 'Preset ' + x;
    }
}
...
function savePerkPreset() {
    ...
    for (var item in game.portal) {
        var temp = game.portal[item].levelTemp
...
function importPerks() {
    ...
    for (var perk in game.portal) {
        if (!levels[perk]) {
            if (portalUniverse == 1 && game.portal[perk].locked !== false)
                continue;
        ...
        if (level < game.portal[perk].level) respecNeeded = true;
...
function removePerk(what) {
    ...
    if (toBuy.levelTemp + perkLevel == 0) {
        ...
        if (roundingError) {
            console.log(
                'rounding error of ' + roundingError + ', adding to refund',
            );
                ...
                (1 +
                    game.portal.Carpentry_II.modifier *
                        (getPerkLevel('Carpentry_II') +
                            game.portal.Carpentry_II.levelTemp)),
        );
...
function loadEquipment(oldEquipment) {
    ...
    for (var item in oldEquipment) {
        ...
        if (oldEquip.blockNow) {
            ...
            stat = typeof newEquip.health !== 'undefined' ? 'health' : 'attack';
        ...
        if (newEquip[stat + 'Calculated'] != oldEquip[stat + 'Calculated']) {
            var dif =
                newEquip[stat + 'Calculated'] - oldEquip[stat + 'Calculated'];
...
function getMaxForResource(what) {
    ...
    var newMax =
        res.max +
        res.max * game.portal.Packrat.modifier * getPerkLevel('Packrat');
    newMax = calcHeirloomBonus('Shield', 'storageSize', newMax);
    return newMax;
}
...
function setGatherTextAs(what, on) {
    ...
    switch (what) {
        ...
        case 'trimps':
            return on
                ? 'Trapping ' + trimpTrapText
                : 'Check Traps ' + trimpTrapText;
    }
}
    ...
    for (var job in game.jobs) {
        ...
        if (game.jobs[job].owned > 0) {
            ...
            if (getPerkLevel('Motivation') > 0)
                perSec +=
                    perSec *
                    getPerkLevel('Motivation') *
                    game.portal.Motivation.modifier;
            if (getPerkLevel('Motivation_II') > 0)
                perSec *=
                    1 +
                    getPerkLevel('Motivation_II') *
            ...
            if (game.global.challengeActive == 'Daily') {
                if (
                    typeof game.global.dailyChallenge.dedication !== 'undefined'
                )
        ...
        if (game.resources[increase].max > 0) {
            var timeToFillElem = document.getElementById(
                increase + 'TimeToFill',
            );
            if (timeToFillElem)
...
function calculateTimeToMax(resource, perSec, toNumber, fromGather) {
    ...
    if (toFill < 60) {
        ...
        return (
            Math.floor(seconds) + ' Sec' + (Math.floor(seconds) == 1 ? '' : 's')
        );
    }
    if (toFill < 3600)
        return (
...
function canAffordBuilding(
    ...
    if (forceAmt) purchaseAmt = forceAmt;
        ...
        if (game.global.buyAmt == 'Max') {
            if (!updatingLabel)
                purchaseAmt = calculateMaxAfford(
                    toBuy,
                    !isEquipment,
                    isEquipment,
                );
        ...
        if (buildCostString) {
            ...
            if (color == 'red') {
                ...
                if (thisPs > 0) {
                    ...
                    percent = "(<span class='icomoon icon-infinity'></span>)";
            } else {
                percent =
        ...
        if (typeof thisCostItem[1] !== 'undefined')
            refund = Math.floor(
                thisCostItem[0] *
                    ...
                    ((Math.pow(thisCostItem[1], name[1]) - 1) /
                        (thisCostItem[1] - 1)),
            );
...
function buildBuilding(what) {
    ...
    if (typeof building.increase !== 'undefined') {
        if (building.increase.what == 'trimps.max') {
            addMaxHousing(
                building.increase.by,
                game.talents.autoStructure.purchased,
            );
        } else {
            var buildingSplit = building.increase.what.split('.');
            if (buildingSplit[0] == 'global') toIncrease = game.global;
            else if (buildingSplit[0] == 'Dragimp')
                toIncrease = game.jobs.Dragimp;
            ...
            if (buildingSplit[2] == 'mult')
                ...
                toIncrease[buildingSplit[1]] += parseFloat(
                    building.increase.by,
                );
        }
    }
...
function trapThings() {
    ...
    game.global.timeLeftOnTrap -=
        (1 / game.settings.speed) * getPlayerModifier();
    if (
...
function autoBuyJob(what, isRatio, purchaseAmt, max) {
    ...
    for (var costItem in job.cost) {
        if (checkJobItem(what, true, costItem, null, buyAmt) !== true)
            return false;
    }
    job.owned += buyAmt;
    if (checkAndFix) {
...
function buyJob(what, confirmed, noTip) {
    ...
    var checkAndFix = false;
    if (game.options.menu.pauseGame.enabled) return;
    if (game.global.challengeActive == 'Scientist' && what == 'Scientist')
        return;
    ...
    for (var item in itemObj.cost) {
        ...
        if (typeof price[1] !== 'undefined') {
            ...
            toBuy = Math.floor(
                log10(
                    (resourcesAvailable /
                        (start * Math.pow(price[1], currentOwned))) *
                        (price[1] - 1) +
                        1,
                ) / log10(price[1]),
            );
...
function canAffordJob(what, take, workspaces, updatingLabel, fromAuto) {
    ...
    var job = game.jobs[what];
    for (var costItem in job.cost) {
        if (checkJobItem(what, take, costItem, null, toBuy) !== true)
            return false;
    }
...
function checkJobItem(what, take, costItem, amtOnly, toBuy) {
    ...
    if (
        game.resources[costItem].max > 0 &&
        getMaxForResource(costItem) < price
    ) {
        return 0;
    }
    if (game.resources[costItem].owned < price) {
...
function breed() {
    ...
    var timeRemaining = DecimalBreed.log10(
        maxBreedable.div(decimalOwned.minus(trimps.employed)),
    )
    ...
    if (
        ...
        if (GAElem && canRun) {
            ...
            if (
                ...
                timeRemaining.cmp(target.add(1)) > 0
            ) {
            ...
            if (compareTime.cmp(target) < 0) {
                ...
                if (
                    game.resources.food.owned * 0.01 <
    ...
    if (decimalOwned.cmp(trimpsMax) >= 0 && trimps.owned >= trimpsMax) {
        ...
        if (
            ...
            game.global.lastBreedTime / 1000 <
...
function customizeGATargets() {
    ...
    for (var x = 1; x <= 3; x++) {
        ...
        if (toKeep.indexOf(val) != -1) {
            error =
                val + ' cannot be used twice. Please choose unique numbers!';
            break;
        }
        if (isNumberBad(val)) {
            error =
...
function prestigeEquipment(what, fromLoad, noInc) {
    ...
    cost[0] = Math.round(
        equipment.oc *
            Math.pow(1.069, prestigeMod * game.global.prestige.cost + 1),
    );
    var stat;
    ...
    if (!fromLoad)
        game.global.difs[stat] -=
            equipment[stat + 'Calculated'] * equipment.level;
    equipment[stat + 'Calculated'] = Math.round(
        equipment[stat] *
            Math.pow(
                1.19,
                (equipment.prestige - 1) * game.global.prestige[stat] + 1,
            ),
    );
...
function createMap(
    ...
    var newMap = {
        ...
        size: sizeOverride
            ? sizeOverride
            : Math.floor(getRandomMapValue('size')),
...
function resetAdvMaps(fromClick) {
    ...
    if (
        game.global.decayDone &&
        document.getElementById('gardenOption') === null
    )
        biomeElem.innerHTML +=
...
function getMapSliderValue(what) {
    ...
    var val = parseInt(
        document.getElementById(what + 'AdvMapsRange').value,
        10,
    );
...
function updateMapCost(getValue, forceBaseCost) {
    ...
    document.getElementById('mapCostFragmentCost').innerHTML = prettify(
        baseCost,
    );
    if (getHighestLevelCleared() >= getUnlockZone('extra'))
...
function buyMap() {
    ...
    if (cost > 0 && game.resources.fragments.owned >= cost) {
        ...
        message(
            "You can't afford this map! You need " +
                prettify(cost) +
                ' fragments.',
            'Notices',
        );
...
function createVoidMap(forcePrefix, forceSuffix, skipMessage) {
    ...
    if (game.global.challengeActive == 'Mapocalypse')
        profiles[2] =
            profiles[2] + game.challenges.Mapocalypse.difficultyIncrease;
...
var voidBuffConfig = {
    doubleAttack: {
        ...
        text:
            'This Bad Guy attacks twice - once before you, and once again after you.',
        title: 'Void Attack',
    },
    slowBreed: {
        icon: 'icomoon icon-cloudy2',
        text:
            'This map is reducing the repopulation speed of your Trimps by 80%.',
        title: 'Void Gas',
    },
    getCrit: {
        icon: 'icomoon icon-heart6',
        text:
            'This Bad Guy has a 25% chance to crit you for 400% extra damage.',
        title: 'Void Strength',
    },
    bleed: {
        icon: 'icomoon icon-drop',
        text:
            'Every time this Bad Guy attacks, you will lose an additional 20% of your <b>current</b> health.',
        title: 'Void Bleed',
    },
};
...
function populateHeirloomWindow() {
    ...
    if (
        game.stats.coresFound.value > 0 ||
        game.stats.coresFound.valueTotal > 0
    ) {
        ...
        document.getElementById(
            'CoreEquipped',
        ).innerHTML = generateHeirloomIcon(
    ...
    document.getElementById('recycleAllHeirloomsBtn').style.display = game
        .global.heirloomsExtra.length
        ? 'inline-block'
        : 'none';
...
function displayAddCarriedBtn() {
    ...
    document.getElementById('carriedHeirloomsText').innerHTML =
        ' - You can carry <b>' +
        realMaxHeirlooms +
    ...
    if (game.global.nullifium < cost)
        swapClass('heirloomBtn', 'heirloomBtnInactive', elem);
...
function addCarried(confirmed) {
    ...
    if (!confirmed) {
        tooltip(
            ...
            null,
            'update',
            'You are about to purchase 1 extra slot to carry Heirlooms through the Portal for ' +
                cost +
                ' Nullifium. Are you sure?',
            'addCarried(true)',
            'Upgrade Carried Slots',
        );
        return;
    }
...
function displayExtraHeirlooms() {
    ...
    if (game.global.spiresCompleted >= 1)
        heirloomExtraText +=
            ' and ' + recycleAllExtraHeirlooms(false, true) + ' Ss';
...
function selectHeirloom(number, location, noScreenUpdate) {
    ...
    switch (location) {
        ...
        case 'CoreEquipped':
            document.getElementById(
                'equippedHeirloomsBtnGroup',
            ).style.visibility = 'visible';
            break;
        case 'heirloomsCarried':
            document.getElementById(
                'carriedHeirloomsBtnGroup',
            ).style.visibility = 'visible';
            document.getElementById('equipHeirloomBtn').innerHTML =
                typeof game.global[heirloom.type + 'Equipped'].name ===
                'undefined'
                    ? 'Equip'
                    : 'Swap';
            break;
...
function equipHeirloom(noScreenUpdate) {
    ...
    for (var item in heirloom.mods) {
        ...
        game.heirlooms[heirloom.type][
            heirloom.mods[item][0]
        ].currentBonus = bonus;
        if (name == 'trimpHealth') {
...
function generateHeirloomIcon(heirloom, location, number) {
    if (typeof heirloom.name === 'undefined')
        return "<span class='icomoon icon-sad3'></span>";
...
function displaySelectedHeirloom(
    ...
    html += ' ' + res + ' Spent';
    if (heirloom.type != 'Core')
        html += ' - ' + prettify(Math.max(allowed - spent, 0)) + ' Unspent';
    html += '</span>';
    html += '<br/>';
    var srText =
        'Selected ' +
    ...
    for (var x = 0; x < heirloom.mods.length; x++) {
        ...
        if (heirloom.mods[x][0] == 'empty') {
            html +=
                '- <span role="button" class="heirloomMod heirloomModEmpty" ';
    ...
    if (fromPopup) {
        ...
        document.getElementById('heirloomsPopup').style.display =
            'inline-block';
        return;
    }
...
function selectMod(which, fromPopup) {
    ...
    var resourceCount =
        ...
        getTotalHeirloomRefundValue(heirloom, true);
    if (isCore) {
        resourceShort = 'Ss';
    ...
    upgradeBtn.innerHTML =
        'Upgrade x1<br/>(' + prettify(upgradeCost) + ' ' + resourceShort + ')';
    upgradeBtn10.innerHTML =
        'Upgrade x10<br/>(' +
        prettify(upgradeCost10) +
        ' ' +
        resourceShort +
        ')';
...
function getModUpgradeCost(heirloom, modIndex, count) {
    ...
    if (
        modConfig.max &&
        getModUpgradeValue(heirloom, modIndex, count) === false
    )
        return Infinity;
    var cost = 0;
...
function countPriceOfUpgrades(dummyHeirloom, count) {
    ...
    for (var x = 0; x < count; x++) {
        ...
        dummyHeirloom.mods[0][1] =
            Math.round(dummyHeirloom.mods[0][1] * 100) / 100; //Damn you, javascript
    }
    return total;
}
    ...
    if (!confirmed && game.options.menu.boneAlerts.enabled == 1) {
        tooltip(
            ...
            'You are about to upgrade ' +
                game.heirlooms[heirloom.type][heirloom.mods[selectedMod][0]]
                    .name +
                (count > 1 ? ' 10 times' : '') +
                ' for ' +
                prettify(cost) +
...
function replaceMod(confirmed) {
    ...
    if (!confirmed && game.options.menu.boneAlerts.enabled == 1) {
        ...
        text +=
            newMod.name + ' for ' + prettify(cost) + ' ' + resourceLong + '. ';
        text += ' Are you sure?';
    ...
    mod[0] = newModName;
    if (!heirloom.repSeed)
        heirloom.repSeed = getRandomIntSeeded(
            game.global.heirloomSeed,
            1,
            10e6,
        );
...
function updateHeirloomSpirestoneCount() {
    document.getElementById('heirloomSpirestoneCount').innerHTML =
        '&nbsp;and <b>' +
        prettify(playerSpire.spirestones) +
        '</b> Spirestones';
}
...
function buildModOptionDdl(type, rarity, selectedMod) {
    if (!game.heirlooms.canReplaceMods[rarity] && selectedMod != 'empty') {
        ...
        document.getElementById('modReplaceSelect').style.display =
            'inline-block';
    ...
    for (var item in game.heirlooms[type]) {
        ...
        if (typeof thisMod.filter !== 'undefined' && !thisMod.filter())
            continue;
...
function setHeirRareText(forBones) {
    var rarityBreakpoint = forBones
        ? getHeirloomZoneBreakpoint(game.global.highestLevelCleared + 1)
    ...
    if (!forBones) {
        ...
        if (breakpoint == game.heirlooms.rarityBreakpoints.length)
            ...
            nextAt =
                ...
                game.heirlooms.rarityBreakpoints[breakpoint];
            if (
                game.heirlooms.universeBreakpoints[breakpoint] >
    ...
    if (forBones)
        document.getElementById('heirloomRarityMisc').innerHTML = html;
...
function createHeirloom(zone, fromBones, spireCore) {
    ...
    for (var item in game.heirlooms[type]) {
        ...
        if (typeof heirloom.filter !== 'undefined' && !heirloom.filter())
            continue;
...
function getHeirloomRarityRanges(zone, forBones) {
    ...
    var rarities =
        game.heirlooms.rarities[getHeirloomZoneBreakpoint(zone, forBones)];
    var canLower = 0;
    var addBonus = false;
    if (
...
function getHeirloomRarity(zone, seed, fromBones) {
...
function updateEmpowerCosts() {
    for (var item in game.empowerments) {
        ...
        document.getElementById(
            'natureUpgrade' + item + 'Cost',
        ).innerHTML = checkAndFormatTokens(getNextNatureCost(item), item);
        document.getElementById(
            'natureStackTransfer' + item + 'Cost',
        ).innerHTML =
            emp.retainLevel >= 80
        ...
        var purchaseAmount = ctrlPressed
            ? Math.floor(emp.tokens / 10) * 10
            : 10;
        for (var itemY in game.empowerments) {
            if (itemY == item) continue;
            document.getElementById(
                'nature' + item + itemY + 'Cost',
            ).innerHTML = checkAndFormatTokens(purchaseAmount, item);
        }
    }
}
...
function natureTooltip(event, doing, spending, convertTo) {
    ...
    if (doing == 'upgrade') {
        ...
        if (game.empowerments[spending].getLevel() < 50) {
            tipTitle = 'Locked';
            tipText =
                'Upgrade Empowerment of ' +
                spending +
                ' to Level 50 to unlock.';
            tipCost = 0;
        } else {
...
function updateNatureInfoSpans() {
    for (var item in game.empowerments) {
        ...
        document.getElementById(
            'natureStackTransfer' + item + 'Level',
        ).innerHTML = 'Lv: ' + prettify(emp.retainLevel + bonusLevels);
...
var mutations = {
    Living: {
        ...
        change: function() {
            ...
            if (
                ...
                (game.challenges.Life.arrayHolder.length == 4 &&
        ...
        savePattern: function(newArray, reset) {
            ...
            if (game.challenges.Life.arrayHolder.length > 4)
                game.challenges.Life.arrayHolder = game.challenges.Life.arrayHolder.slice(
                    0,
                    4,
                );
        },
                ...
                if (isAlive && !wasAlive) {
                    ...
                    if (game.global.lastClearedCell + 1 == y) {
                        var elem = document.getElementById(
                            'livingMutationContainer',
                        );
                        if (elem != null) {
                            document.getElementById(
                                'livingMutationContainer',
                            ).className = 'badNameMutation Living';
                            document.getElementById(
                                'livingMutationName',
                            ).innerHTML = 'Living ';
                        }
        ...
        nextMove: function(currentArray) {
            ...
            for (var x = 0; x < lastPattern.length; x++) {
                ...
                for (var y = 0; y < directions.length; y++) {
                    if (
                        lastPattern[this.checkDirection(directions[y], x)] ==
                        'Living'
                    )
                        neighborCount++;
                }
    ...
    Corruption: {
        ...
        cellCount: function() {
            var possible =
                Math.floor((game.global.world - this.start()) / 3) + 2;
        ...
        attack: function(cellNum, name) {
            return (
                game.global.getEnemyAttack(cellNum, name, true) *
                this.statScale(3)
            );
        },
        health: function(cellNum, name) {
            return (
                game.global.getEnemyHealth(cellNum, name, true) *
                this.statScale(10)
            );
        },
        ...
        reward: function(effect) {
            if (game.global.world < 20 || game.global.runningChallengeSquared)
                return;
            var percentage =
    ...
    Magma: {
        ...
        addPrettyRiver: function(currentArray, origin, length) {
            ...
            for (i = 0; i < 100; i++) {
                branchLength = getRandomIntSeeded(
                    ...
                    length > this.maxBranchLength
                        ? this.maxBranchLength + 1
                        : length + 1,
                );
                ...
                for (j = 0; j < 4; j++) {
                    pathDone = this.addBranch(
                        currentArray,
                        arr,
                        branchLength,
                        dirArr[0],
                    ); //try doing a branch in one of the directions from the array of directions
                    if (pathDone > 0) {
        ...
        generateRivers: function(currentArray) {
            ...
            for (var i = 0; i < 20; i++) {
                if (targetCells > 0) {
                    ...
                    var newTarget =
                        targetCells > singlePathMaxSize
                            ? singlePathMaxSize
                            : targetCells; //determine target river length
        ...
        pattern: function(currentArray) {
            ...
            var threshold = this.discardMultiplePaths
                ? this.discardMaxThreshold
                : 1;
            ...
            for (i = 0; i <= threshold; i++) {
                ...
                if (
                    rivers ==
                    Math.ceil(this.targetCells / this.singlePathMaxSize)
                )
                    break;
                else if (i != threshold) {
            ...
            for (i = 0; i < tempCurrentArray.length; i++) {
                if (
                    tempCurrentArray[i] == 'Corruption' &&
                    currentArray[i] == 'Magma'
                )
                    replacedCorruptions++;
            }
            ...
            if (
                ...
                (game.global.generatorMode == 1 ||
                    (game.global.generatorMode == 2 &&
                        getGeneratorFuelCap(false, true) -
    ...
    Obsidian: {
        ...
        tooltip: function() {
            var text =
                'This enemy is rock solid, and there is no way to get past.';
    ...
    Healthy: {
        ...
        tooltip: function(effectName) {
            ...
            text +=
                ' It will also drop 45% of the helium you would normally get from completing this Zone.';
            return text;
        },
        effects: [
            'healthyDbl',
...
var mutationEffects = {
    ...
    healthyDbl: {
        icon: 'icomoon icon-pushpin',
        text:
            'this Bad Guy attacks twice for 1.5x damage - once before you, and once again after you.',
        title: 'Healthy and Tireless',
    },
    healthyCrit: {
        icon: 'icomoon icon-heart6',
        text:
            'this Bad Guy has a 25% chance to crit you for 600% extra damage.',
        title: 'Healthy and Precise',
    },
    healthyBleed: {
        icon: 'icomoon icon-drop',
        text:
            'every time this Bad Guy attacks, you will lose an additional 30% of your <b>current</b> health.',
...
function canAffordGeneratorUpgrade() {
    ...
    for (var permUnlock in game.permanentGeneratorUpgrades) {
        if (
            ...
            game.global.magmite >=
                game.permanentGeneratorUpgrades[permUnlock].cost
        )
            return true;
    }
    return false;
...
function updateGeneratorInfo() {
    ...
    if (elem == null) {
        document.getElementById('buildingsHere').innerHTML += getGeneratorHtml(
            true,
        );
    }
    changeGeneratorState(null, true);
...
function getGeneratorHtml(getContainer) {
    ...
    html +=
        ...
        (game.permanentGeneratorUpgrades.Supervision.owned
            ...
            : '') +
        "> <div id='generatorRadialContainer' class='radial-progress'> <div class='radial-progress-circle'> <div class='radial-progress-arrow static''></div></div><div id='generatorRadial' class='radial-progress-circle'> <div class='radial-progress-arrow mobile'></div> </div> <div id='clockKnob' class='radial-progress-knob generatorState'></div></div><span id='generatorNextTick' style='pointer-events: none;'>0</span></div></div></div></div>";
...
function scaleNumberForBonusHousing(num) {
    ...
    if (
        ...
        num = Math.floor(
            num *
                dailyModifiers.large.getMult(
                    game.global.dailyChallenge.large.strength,
                ),
        );
    if (game.global.challengeActive == 'Size') num *= 0.5;
    return num;
}
...
function updateGeneratorUpgradeHtml() {
    ...
    if (document.getElementById('magmiteCost') != null && lastViewedDGUpgrade)
        showGeneratorUpgradeInfo(
            lastViewedDGUpgrade[0],
            lastViewedDGUpgrade[1],
        );
}
        ...
        Math.ceil(
            (1 /
                Math.pow(
                    zoneMult,
                    Math.floor(
                        (game.global.world - mutations.Magma.start()) / 3,
                    ),
                )) *
                baseTick *
                10,
...
function updateGeneratorFuel() {
    ...
    document.getElementById('generatorFuelOwned').innerHTML = prettify(
        currentFuel,
    );
...
function generatorTick(fromOverclock) {
    ...
    if (fromOverclock)
        tickAmt *= 1 - game.generatorUpgrades.Overclocker.modifier;
...
function buildGrid() {
    ...
    for (var i = 0; i < 100; i++) {
        ...
        if (
            (i >= 50 &&
                needsEmpower &&
                typeof newCell.corrupted === 'undefined') ||
            (i == 98 && needsEmpower)
        ) {
...
function getRandomBadGuy(
    ...
    for (var item in game.badGuys) {
        ...
        if (
            ...
            if (item == 'Blimp' && world != 5 && world != 10 && world < 15)
                continue;
            if (
                !mapSuffix &&
    ...
    if (!force)
        selected =
            badGuysArray[
                getRandomIntSeeded(enemySeed++, 0, badGuysArray.length)
            ];
    if (!mapSuffix) game.global.enemySeed = enemySeed;
    return selected;
}
...
function addSpecials(maps, countOnly, map, getPrestiges) {
    ...
    for (var item in unlocksObj) {
        ...
        if (
            maps &&
            special.filter &&
            game.mapConfig.locations[map.location].resourceType != item
        )
            continue;
        if (typeof special.specialFilter !== 'undefined') {
            if (!special.specialFilter(world)) continue;
        ...
        if (
            ...
            if (
                special.prestige &&
                maps &&
                game.options.menu.mapLoot.enabled == 0
            ) {
...
function findHomeForSpecial(special, item, array, max) {
    ...
    while (done === false) {
        if (special.prestige) level = array.length - 2;
            ...
            level =
                Math.floor(
                    Math.random() * (special.level[1] - special.level[0]),
                ) +
                special.level[0] +
                x * repeatFreq;
...
function drawGrid(maps) {
    ...
    for (var i = 0; i < rows; i++) {
        ...
        for (var x = 0; x < cols; x++) {
            ...
            if (!maps && game.global.gridArray[counter].mutation)
            ...
            if (!maps && game.global.gridArray[counter].empowerment) {
                className +=
                    ' empoweredCell' +
                    game.global.gridArray[counter].empowerment;
                cell.title =
                    'Token of ' + game.global.gridArray[counter].empowerment;
            ...
            if (
                !maps &&
                game.global.gridArray[counter].special == 'easterEgg'
            ) {
...
function recycleBelow(confirmed) {
    ...
    for (var x = game.global.mapsOwnedArray.length - 1; x >= 0; x--) {
        ...
        if (game.global.currentMapId == item.id && game.global.mapsActive)
            continue;
...
function recycleMap(map, fromMass, killVoid, noRefund) {
    ...
    document
        .getElementById(loc)
        .removeChild(document.getElementById(mapObj.id));
...
function mapsClicked(confirmed) {
    ...
    if (game.global.fighting && !game.global.preMapsActive) {
        message(
            'Waiting to travel until your soldiers are finished.',
            'Notices',
        );
...
function mapsSwitch(updateOnly, fromRecycle) {
    ...
    if (game.global.preMapsActive) {
        ...
        if (
            currentMapObj &&
            (currentMapObj.location == 'Void' ||
                currentMapObj.location == 'Darkness')
        ) {
        ...
        if (game.global.lookingAtMap && !game.global.currentMapId)
            ...
            document.getElementById('selectMapBtn').style.visibility =
                'visible';
        ...
        if (currentMapObj.location == 'Void') {
            ...
            document.getElementById('repeatVoidsContainer').style.display =
                'block';
        } else
            document.getElementById('repeatVoidsContainer').style.display =
                'none';
        ...
        if (currentMapObj.location == 'Bionic') {
            ...
            document.getElementById('climbBwContainer').style.display = 'none';
...
function setNonMapBox() {
    ...
    if (game.global.mapBonus > 0)
        mapBonus.innerHTML =
            prettify(game.global.mapBonus * 20) + '% Map Bonus';
...
function selectMap(mapId, force) {
    ...
    if (
        typeof game.global.mapsOwnedArray[
...
function runMap() {
    ...
    if (game.global.challengeActive == 'Watch')
        game.challenges.Watch.enteredMap = true;
    if (
        game.global.challengeActive == 'Mapology' &&
        !game.global.currentMapId
    ) {
...
function battleCoordinator(makeUp) {
    ...
    var num = getPerkLevel('Agility')
        ? 1000 *
          Math.pow(1 - game.portal.Agility.modifier, getPerkLevel('Agility'))
        : 1000;
...
function startFight() {
        ...
        if (cellElem == null) {
            ...
            if (game.global.lastClearedCell != 99) {
                if (game.global.lastClearedCell == -1) {
                    ...
                    document.getElementById('metal').style.visibility =
                        'visible';
                    console.log(
    ...
    if (mutations.Living.active()) {
        badName =
            ...
            (cell.mutation == 'Living'
                ? " class='badNameMutation Living'"
                : '') +
    ...
    if (cell.maxHealth == -1) {
        ...
        if (
            ...
            cell.attack = mutations[cell.mutation].attack(
                cell.level,
                cell.name,
            );
        ...
        if (
            ...
            cell.health = mutations[cell.mutation].health(
                cell.level,
                cell.name,
            );
        ...
        if (cell.empowerment) {
            if (cell.mutation != 'Corruption') {
                cell.health = mutations.Corruption.health(
                    cell.level,
                    cell.name,
                );
                cell.attack = mutations.Corruption.attack(
                    cell.level,
                    cell.name,
                );
            }
        ...
        if (game.global.challengeActive == 'Daily') {
            ...
            if (
                typeof game.global.dailyChallenge.badMapHealth !==
                    'undefined' &&
                game.global.mapsActive
            ) {
        ...
        if (game.global.mapsActive) {
            ...
            if (game.global.world >= corruptionStart) {
                if (mutations.Magma.active() && map.location == 'Void') {
                    ...
                    cell.health *= mutations.Corruption.statScale(10).toFixed(
                        1,
                    );
                } else if (map.location == 'Void' || mutations.Magma.active()) {
                    cell.attack *= (
                        mutations.Corruption.statScale(3) / 2
                    ).toFixed(1);
                    cell.health *= (
                        mutations.Corruption.statScale(10) / 2
                    ).toFixed(1);
                }
            }
        }
        if (game.global.challengeActive == 'Meditate') cell.health *= 2;
            ...
            cell.health *=
                1 + Math.min(game.challenges.Lead.stacks, 200) * 0.04;
        ...
        if (cell.name == 'Improbability' || cell.name == 'Omnipotrimp') {
            if (game.global.roboTrimpLevel && game.global.useShriek)
                activateShriek();
            if (game.global.world >= corruptionStart) {
                if (game.global.spireActive) {
        ...
        if (cell.health < 1) {
            ...
            if (cell.OKcount <= overkillerCount) {
                ...
                if (nextCell) {
                    nextCell.health =
                        overkill == 'shatter' &&
                        (cellNum != 98 || !game.global.spireActive)
                            ? 'shatter'
            ...
            if (cell.plaguebringer) {
                ...
                if (empowerment) {
                    ...
                    if (empowerment == 'Wind') {
                        ...
                        game.empowerments[
                            empowerment
                        ].currentDebuffPower += Math.ceil(hits);
                        handleWindDebuff();
                    }
                    if (empowerment == 'Ice') {
                        var hits = cell.plagueHits;
    ...
    if (game.global.soldierHealth <= 0) {
        ...
        if (
            game.global.realBreedTime >= 600000 &&
            game.jobs.Geneticist.owned >= 1
        )
        ...
        if (getPerkLevel('Anticipation')) {
            game.global.antiStacks =
                game.jobs.Amalgamator.owned > 0
                    ? Math.floor(
                          (getGameTime() - game.global.lastSoldierSentAt) /
                              1000,
                      )
            ...
            } else if (game.global.antiStacks >= 30)
        ...
        if (getPerkLevel('Toughness_II') > 0)
            game.global.soldierHealthMax *=
                1 +
                game.portal.Toughness_II.modifier *
                    getPerkLevel('Toughness_II');
        ...
        if (
            ...
            if (freeTrimps >= requiredTrimps) {
                ...
                game.global.soldierHealth +=
                    game.global.soldierHealthMax - oldHealth;
        ...
        if (game.global.difs.health !== 0) {
            var healthTemp =
                ...
                (game.portal.Toughness.modifier * getPerkLevel('Toughness') +
                    1);
            if (mutations.Magma.active()) {
        ...
        if (game.global.difs.attack !== 0) {
            ...
            if (getPerkLevel('Power_II'))
                attackTemp *=
                    1 +
                    game.portal.Power_II.modifier * getPerkLevel('Power_II');
            if (game.global.formation !== 0 && game.global.formation != 5) {
...
function updateAllBattleNumbers(skipNum) {
    ...
    document.getElementById('goodGuyBlock').innerHTML =
        game.global.universe == 2
            ? prettify(game.global.soldierEnergyShieldMax) +
              ' (' +
              Math.round(getEnergyShieldMult() * 100) +
              '%)'
            : prettify(game.global.soldierCurrentBlock);
...
function toZalgo(string, seed, strength) {
    ...
    for (var x = 0; x < string.length; x++) {
        ...
        if (rand < strength) {
            newString +=
                zalgo_table[
                    getRandomIntSeeded(seed + x, 0, zalgo_table.length)
                ];
        }
    }
    return newString;
...
function calculateDamage(
    ...
    if (isTrimp) {
        ...
        if (game.global.challengeActive == 'Daily') {
            ...
            if (
                typeof game.global.dailyChallenge.oddTrimpNerf !==
                    'undefined' &&
                game.global.world % 2 == 1
            ) {
            ...
            if (
                typeof game.global.dailyChallenge.evenTrimpBuff !==
                    'undefined' &&
                game.global.world % 2 == 0
            ) {
        ...
        if (game.global.challengeActive) {
            ...
            if (game.global.challengeActive == 'Daily') {
                if (
                    typeof game.global.dailyChallenge.badStrength !==
                    'undefined'
                ) {
                ...
                if (
                    typeof game.global.dailyChallenge.badMapStrength !==
                        'undefined' &&
                    game.global.mapsActive
                ) {
                ...
                if (
                    typeof game.global.dailyChallenge.bloodthirst !==
                    'undefined'
                ) {
    ...
    if (buildString || runningUnlucky) {
        ...
        if (isTrimp) {
            ...
            if (!buildString && isTrimp) {
                ...
                if (Number(critMin.toString()[0]) % 2 == 0)
                    actuallyLucky = true;
...
function tryScry() {
    ...
    if (
        ...
        message(
            'You found ' + prettify(reward) + ' Dark Essence!',
            'Loot',
            '*cloud3',
            'essenceMessage',
            'essence',
...
function respecTalents(confirmed, force) {
    ...
    for (var item in game.talents) {
        ...
        if (
            wasPurchased &&
            typeof game.talents[item].afterRespec === 'function'
        )
...
function rewardLiquidZone() {
    ...
    for (var x = 1; x < 100; x++) {
        ...
        if (
            cell.mutation &&
            typeof mutations[cell.mutation].reward !== 'undefined'
        )
    ...
    var addRepeateds =
        unlocks[1] !== '' && game.global.messages.Unlocks.repeated;
    ...
    if (
        game.global.messages.Loot.enabled &&
        (game.global.messages.Loot.primary ||
            game.global.messages.Loot.secondary)
    ) {
        ...
        if (game.global.messages.Loot.primary) {
            text +=
                ' Food - ' + prettify(game.resources.food.owned - food) + ',';
            text +=
                ' Wood - ' + prettify(game.resources.wood.owned - wood) + ',';
            text +=
                ' Metal - ' +
                prettify(game.resources.metal.owned - metal) +
                ',';
        }
...
function nextWorld() {
    ...
    if (game.talents.explorers.purchased) {
        if (
            Math.floor(
                (game.global.world - game.mapUnlocks.Speedexplorer.next) / 10,
            )
        ) {
    ...
    if (
        ...
        if (
            game.stats.cellsOverkilled.value +
                game.stats.zonesLiquified.value * 50 ==
            2950
        )
    ...
    if (game.global.challengeActive == 'Life') {
        if (
            ...
            game.challenges.Life.lowestStacks == 150
        )
...
function checkMapAtZoneWorld(runMap) {
    if (
        ...
        for (
            var x = 0;
            x < game.options.menu.mapAtZone.getSetZone().length;
            x++
        ) {
            if (
                game.global.world ==
                game.options.menu.mapAtZone.getSetZone()[x].world
            ) {
                if (runMap) {
                    runMapAtZone(x);
                    return;
...
function runMapAtZone(index) {
    ...
    if (mapStatus == 1) {
        if (game.global.currentMapId) recycleMap();
        selectMap(
            game.global.mapsOwnedArray[game.global.mapsOwnedArray.length - 1]
                .id,
        );
        runMap();
    }
...
function purgeBionics() {
    ...
    if (bionicMaps.length > 3) {
        ...
        for (var x = 0; x < trim; x++) {
            ...
            if (
                game.global.mapsOwnedArray[index].id == game.global.currentMapId
            ) {
...
function startSpire(confirmed) {
    ...
    if (!confirmed) {
        ...
        if (spireSetting && !checkMapAtZoneWorld()) {
            var highestSpire = Math.floor(
                (getHighestLevelCleared() - 99) / 100,
            );
            if (
                spireSetting == 1 ||
...
function getSpireStory(spireNum, row) {
    var spires = {
        ...
        spire4: {
            r2:
                "As you guessed before you even stepped in, this Spire is in even worse condition than the one before. The air is colder, the walls are more still, and there's a feeling of vacancy. Might as well look for stuff to take! ",
            r4:
                "You wonder if the Echo of Druopitee will even be able to speak anymore. You know he'll be at the top waiting, just like the infinite other Spires, but maybe he'll just leave you be this time. ",
            r6:
                'The Spire is warming up again. Maybe Druopitee will make his final appearance soon. ',
            r7:
                "<span class='spirePoem'>Please just go around, there's nothing to see<br/>Why is my demise something you must guarantee?<br/>I beg you once more, please hear my plea<br/>We could rule for all time, just you and me</span>No thanks, Druopitee. It's kinda nice to see him scared though! ",
            r8:
                "<span class='spirePoem'>Well here is something I wouldn't normally say<br/>Since it seems like you won't be going away<br/>Each of my Echoes you slay makes my mind decay<br/>I may not be able to communicate after today</span>Yes, that was becoming pretty obvious. His mind must really be decaying fast if he thought this admission would be anything other than motivating. ",
            r9:
                "<span class='spirePoem'>Why don't you care that you're making things harder?<br/>To repair the planet you'd give enemies armor?<br/>When I brought you here, I thought you'd be smarter<br/>But it seems like there's no room left to barter</span>No, there's not. You're almost there. ",
            r10:
        ...
        spire5: {
            r2:
                "While you walk through the Spire, you whistle a tune. You haven't yet tired, your motivations zoom. As Fluffy checks for loot in this dark, dusty room, you suddenly see fire and hear a huge boom. ",
            r6:
                "Against your better judgment, you continue to climb. Heat fills this dungeon, and doubt fills your mind. You feel a great evil that's long been confined, but with reluctance you move on for the good of Trimpkind. ",
            r4:
                'The fires still grow, but they leave a clear path. Just one direction shown, the thought makes you laugh. Unwisely Druopitee tried to rig this booby trap, but the fire shows the way to go so you thank him for the map. ',
            r7:
                "You're close to the top of this large, awful tower. Your fearful thoughts drop, your will is empowered. Luckily you have Fluffy to support you in this hour, you two will never stop until these echoes are devoured.",
            r8:
                "The presence here is unique from what you felt before. You know it can't speak but it's not done for. An intrusive thought knocks on your mind's back door:<br/><span class='spirePoem'>You are weak. I'll be on the top floor.</span> ",
            r9:
                "Another thought enters your mind, this time louder than before:<br/><span class='spirePoem'>Your death warrant's signed, soon you'll be no more.<br/>I gave you everything you have, it was no easy chore.<br/>Your Trimps are MINE, prepare for war!</span><br/>Even in death this guy's a dick. ",
            r10:
        ...
        case 100:
            ...
            if (spireWorld == 6) {
                ...
                if (talentCount < maxTalents) {
                    var maxCost = getTotalTalentCost();
                    if (
                        game.global.spentEssence + game.global.essence <
                        maxCost
                    ) {
            ...
            if (game.global.spiresCompleted < spireWorld) {
                ...
                switch (spireWorld) {
                    ...
                    case 4:
                        text +=
                            ' You have also unlocked the <b>Curious</b> perk!';
                        game.portal.Curious.locked = false;
                        break;
                    case 5:
                        text +=
                            ' You have also unlocked the <b>Classy</b> perk!<b>';
                        game.portal.Classy.locked = false;
                }
            }
            if (
    ...
    switch (level) {
        ...
        case 100:
            ...
            if (game.global.spiresCompleted < 1) {
                ...
                text +=
                    '<br/><br/><b>You were able to properly remove the Core this time, and have found a Common Spire Core Heirloom!</b>';
            }
            text +=
...
function buyGoldenUpgrade(what) {
    ...
    if (
        game.stats.goldenUpgrades.valueTotal +
            game.stats.goldenUpgrades.value >=
            300 &&
        game.global.autoGolden == -1
    ) {
        unlockAutoGolden();
    }
...
var dailyModifiers = {
    ...
    metallicThumb: {
        description: function(str) {
            return (
                'Equipment is ' +
                prettify((1 - this.getMult(str)) * 100) +
                '% cheaper.'
            );
        },
        getWeight: function(str) {
...
function abandonDaily() {
    ...
    if (!isNumberBad(reward)) {
        ...
        console.log(
            'attempted to give ' + reward + ' as daily challenge reward.',
        );
    message(
...
function getDailyTopText(add) {
    ...
    for (var x = 0; x < 7; x++) {
        ...
        var dayDone =
            game.global.recentDailies.indexOf(getDailyTimeString(dayIndex)) !=
            -1;
...
function getDailyChallenge(add, objectOnly, textOnly) {
    ...
    mainLoop: for (var x = 0; x < maxLoops; x++) {
        ...
        modLoop: for (var z = 0; z < maxZLoops; z++) {
            ...
            lookupLoop: for (var y = 0; y < modifierList.length; y++) {
                checkedTotal +=
                    dailyModifiers[modifierList[y]].chance * chanceMod;
            ...
            if (modObj.incompatible) {
                for (var x = 0; x < modObj.incompatible.length; x++) {
                    var incompatibleIndex = modifierList.indexOf(
                        modObj.incompatible[x],
                    );
    ...
    returnText +=
        '</ul>Challenge has no end point, and grants an <u><b>additional ' +
        prettify(getDailyHeliumValue(currentWeight)) +
        '%</b></u> of all ';
...
function getDailyRewardText(portalUni) {
    ...
    if (portalUni == 2) {
        returnText +=
            'Radon, Nu from Heirlooms earned during the run, and Scruffy Exp';
    } else {
        returnText +=
            'Helium' +
        ...
        if (game.global.highestLevelCleared >= 235)
            returnText +=
                ...
                'Nature Tokens';
        if (game.portal.Capable.locked == false)
            returnText += ', and Fluffy Exp';
    }
    return returnText;
}
...
function fight(makeUp) {
    ...
    if (cell.health <= 0 || !isFinite(cell.health)) {
        ...
        if (
            ...
            tryScry();
        if (game.global.challengeActive == 'Nom' && cell.nomStacks == 100)
        ...
        killedText += '!';
        if (usingScreenReader)
            killedText = 'Cell ' + cellNum + ': ' + killedText;
        ...
        if (game.global.challengeActive == 'Lead' && cell.name != 'Liquimp')
        ...
        if (cell.overkilled && game.options.menu.overkillColor.enabled) {
            if (game.options.menu.overkillColor.enabled == 2) {
                var prevCellElem = document.getElementById(
                    (game.global.mapsActive ? 'mapCell' : 'cell') +
                        (cellNum - 1),
                );
                if (prevCellElem)
            ...
            if (game.global.mapsActive) {
                if (typeof game.mapUnlocks[cell.special].last !== 'undefined') {
                    ...
                    if (
                        ...
                        getSLevel() >= 4 &&
                        game.global.challengeActive != 'Mapology' &&
                        Math.ceil(game.mapUnlocks[cell.special].last / 5) % 2 ==
                            1
                    ) {
                        unlock.fire(cell.level);
                        game.mapUnlocks[cell.special].last += 5;
                ...
                if (
                    typeof game.mapUnlocks[cell.special].canRunOnce !==
                    'undefined'
                )
        ...
        if (
            ...
            if (game.options.menu.repeatUntil.enabled == 1 && mapBonusReached)
                shouldRepeat = false;
            else if (
                game.options.menu.repeatUntil.enabled == 2 &&
                allItemsEarned
            ...
            if (
                ...
                game.global.preMapsActive =
                    game.options.menu.exitTo.enabled && nextBw == false
                        ? false
                        : true;
                game.global.mapsActive = false;
                game.global.lastClearedMapCell = -1;
        ...
        if (game.global.soldierHealth > 0) battle(true);
        return;
    }
    ...
    if (
        ...
        game.global.soldierHealth -= dailyModifiers.mirrored.reflectDamage(
    ...
    if (
        ...
        game.global.soldierHealth -=
            game.global.soldierHealthMax *
            (game.challenges.Electricity.stacks * 0.1);
        if (game.global.soldierHealth < 0) thisKillsTheTrimp();
    ...
    if (critTier == -1)
        critSpan.innerHTML = "<span style='color: cyan'>Weak!</span>";
    if (critTier == 1) critSpan.innerHTML = 'Crit!';
    if (critTier == 2)
        critSpan.innerHTML = "<span style='color: orange'>CRIT!</span>";
    if (critTier == 3) {
        critSpan.innerHTML = "<span style='color: red'>CRIT!!</span>";
        redCritCounter++;
    } else if (critTier == 4) {
        critSpan.innerHTML =
            "<span class='critTier4'>CRIT<span class='icomoon icon-atom'></span></span>";
    ...
    if (
        ...
        if (
            ...
            var explodeAndBlock =
                explodeDamage - game.global.soldierCurrentBlock;
            if (explodeAndBlock < 0) explodeAndBlock = 0;
            if (pierce > 0) {
                var explodePierce = pierce * explodeDamage;
                if (explodeAndBlock < explodePierce)
                    explodeAndBlock = explodePierce;
            }
...
function getPlayerCritChance() {
    ...
    if (game.global.challengeActive == 'Daily') {
        if (
            typeof game.global.dailyChallenge.trimpCritChanceUp !== 'undefined'
        ) {
        ...
        if (
            typeof game.global.dailyChallenge.trimpCritChanceDown !==
            'undefined'
        ) {
...
function updateGammaStacks(reset) {
    ...
    var tipText =
        'Your Trimps are charging up for a Gamma Burst! When Charging reaches 5 stacks, your Trimps will release a burst of energy, dealing ' +
        prettify(bonus) +
...
function manageStacks(
    ...
    elem.innerHTML =
        ...
        "', 'customText', event, '" +
        tooltipText +
        '\');" onmouseout="tooltip(\'hide\')"><span id="gammaStack">' +
        stackCount +
        '</span><span class="' +
        icon +
        '"></span></span>';
...
function setFormation(what) {
    if (what) {
        ...
        if (game.global.soldierHealth > 0) {
            ...
            game.global.soldierHealth -=
                oldHealth - game.global.soldierHealthMax;
            if (game.global.soldierHealth <= 0) game.global.soldierHealth = 0;
...
function unlockFormation(what) {
    if (
        ...
        if (game.global.uberNature) {
            ...
            document.getElementById('form5Container').style.display =
                'inline-block';
...
function showBones() {
    ...
    if (fluffyStat.valueTotal > 0 && Fluffy.getLevel() < 10) {
        ...
        heText +=
            ', ' +
            prettify(fluffyStat.valueTotal) +
            ' ' +
            Fluffy.getName() +
            ' Exp';
...
function addBoost(level, previewOnly) {
    ...
    for (var x = 0; x < compatible.length; x++) {
        ...
        document.getElementById(
            job.increase + 'BoostPreview',
        ).innerHTML = prettify(amt);
    }
}

function toggleMinusRes(on) {
    document.getElementById('minusRes').style.display = on ? 'block' : 'none';
    document.getElementById('boostPreview').style.display = on
        ? 'none'
        : 'table';
}
...
function selectBoost(num) {
    ...
    swapClass(
        'boneBtnState',
        'boneBtnStateOff',
        document.getElementById('boost' + other),
    );
    swapClass(
        'boneBtnState',
        'boneBtnStateTeal',
        document.getElementById('boost' + num),
    );
}
...
function kredPurchase(what) {
    ...
    if (what == '0.imports' && boneTemp.bundle.length != 4) {
        hidePurchaseBones();
        document.getElementById('addToBundleRow').style.border =
            '1px solid green';
        return;
    }
...
function givePumpkimpLoot() {
    ...
    if (
        item == 'metal' &&
        (game.global.challengeActive == 'Metal' ||
            game.global.challengeActive == 'Transmute')
    )
        item = 'nothing';
    if (item == 'nothing') {
...
function toggleAutoGolden(noChange) {
    if (!noChange && game.global.autoGolden != -1) {
        ...
        if (game.global.autoGolden == max) game.global.autoGolden = 0;
...
function saveAutoJobsConfig() {
    ...
    for (var x = 0; x < checkboxes.length; x++) {
        ...
        setting[name].value =
            parseFloat(document.getElementById('autoJobSelect' + name).value) /
            100;
...
function validateJobRatios() {
    ...
    for (var x = 0; x < ratios.length; x++) {
        ...
        if (quant < 0) {
            errorElem.innerHTML =
                'Cannot use a number smaller than 0 for ' +
                ratios[x] +
                ' ratio.';
            return false;
        }
...
function saveAutoStructureConfig() {
    ...
    for (var x = 0; x < checkboxes.length; x++) {
        ...
        if (!setting[name]) setting[name] = {};
        setting[name].enabled = checked;
        setting[name].value = document.getElementById(
            'structSelect' + name,
        ).value;
...
function buyAutoEquip() {
    ...
    if (setting.highestTier) {
        for (var item in game.equipment) {
            ...
            if (equip.prestige > highestPrestige)
                highestPrestige = equip.prestige;
        }
    }
    for (var item in game.equipment) {
        if (!setting[item]) continue;
...
function toggleAutoTrap(updateOnly) {
    ...
    if (!updateOnly)
        game.global.trapBuildToggled = !game.global.trapBuildToggled;
...
function autoStorage() {
    ...
    for (var x = 0; x < 3; x++) {
        ...
        if (
            resource.owned >= max * 0.99 &&
            canAffordBuilding(storage[x], false)
        ) {
...
function autoUpgrades() {
    ...
    if (autoPrestigeSetting != 0 && timerCheck)
        autoPrestiges(equipmentAvailable);
}

function autoGoldenUpgrades() {
    if (game.global.autoGolden <= 0) return;
...
function getCheapestPrestigeUpgrade(upgradeArray) {
    ...
    var shieldCost = -1;
    var artMult = getPerkLevel('Artisanistry')
        ? Math.pow(
              1 - game.portal.Artisanistry.modifier,
              getPerkLevel('Artisanistry'),
          )
    ...
    if (
        ...
        var mtMult = dailyModifiers.metallicThumb.getMult(
            game.global.dailyChallenge.metallicThumb.strength,
        );
    ...
    for (var x = 0; x < upgradeArray.length; x++) {
        ...
        var res =
            typeof upgradeObj.cost.resources.metal !== 'undefined'
                ? 'metal'
                : 'wood';
...
var Fluffy = {
    ...
    getFluff: function() {
        ...
        if (this.currentLevel == this.getRewardList().length) {
            possibilities = [
                ...
                name +
                    ' can juggle 3 dozen scientists without breaking a sweat.',
        ...
        return possibilities[
            getRandomIntSeeded(timeSeed, 0, possibilities.length)
        ];
    },
    getExp: function() {
    ...
    calculateLevel: function() {
        var level = Math.floor(
            log10(
                (this.getCurrentExp() / this.getFirstLevel()) *
                    (this.growth - 1) +
                    1,
            ) / log10(this.growth),
        );
    ...
    getExpReward: function(givingExp, count) {
        ...
        var reward =
            (this.baseExp +
                getPerkLevel('Curious') * game.portal.Curious.modifier) *
    ...
    getDamageModifier: function() {
        ...
        if (
            exp[0] >= this.damageModifiers.length ||
    ...
    getBonusForLevel: function(level) {
        ...
        var possible =
            (this.damageModifiers[level] - this.damageModifiers[level - 1]) *
            100 *
            prestigeBonus;
        ...
        if (level == this.currentLevel + 1 && this.getCapableLevel() >= level) {
            ...
            return (
                prettify(earned) + '% / ' + prettify(Math.round(possible)) + '%'
            );
        }
    ...
    refreshTooltip: function(justOnce) {
        ...
        var bottomElem = document.getElementById(
        ...
        switch (what) {
            ...
            case 'daily':
                ...
                text +=
                    game.global.challengeActive == 'Daily'
                        ? ' Currently ' +
                          prettify(
                              1 + getDailyHeliumValue(countDailyWeight()) / 100,
                          ) +
                          '.'
                        : ' Currently 1.';
            ...
            case 'classy':
                elem.innerHTML =
                    ...
                    Math.floor(
                        getPerkLevel('Classy') * game.portal.Classy.modifier,
                    ) +
                    ' thanks to ' +
            ...
            case 'staff':
                elem.innerHTML =
                    ...
                    (1 + getHeirloomBonus('Staff', 'FluffyExp') / 100).toFixed(
                        2,
                    ) +
                    '.';
                return;
    ...
    tooltip: function(big) {
        ...
        if (
            this.getCurrentPrestige() > 0 ||
            this.currentLevel == rewardsList.length
        )
            topText +=
        ...
        if (savedLevel >= rewardsList.length) {
            ...
            topText +=
                prettify(fluffyInfo[1]) +
                ' / ' +
                prettify(fluffyInfo[2]) +
                ' Exp';
            topText += '</span>';
        }
        topText +=
        ...
        if (Fluffy.canGainExp() && game.global.world >= minZoneForExp) {
            topText +=
                '- ' +
                name +
                "'s Exp gain at the end of each Zone is equal to: ";
        ...
        for (var x = 0; x < rewardsList.length; x++) {
            bottomText +=
                fluffyInfo[0] + calculatedPrestige >= x + 1
        ...
        if (countedPrestige > 0) {
            for (var y = 0; y < prestigeRewardsList.length; y++) {
                var levelDisplay =
                    rewardsList.length + (y + 1) - countedPrestige;
                if (levelDisplay > rewardsList.length) continue;
                if (levelDisplay < 0) levelDisplay = 0;
                bottomText +=
                    fluffyInfo[0] >=
                    rewardsList.length - calculatedPrestige + (y + 1)
                        ? "<tr class='fluffyRowComplete'>"
                        : '<tr>';
                levelDisplay =
                    countedPrestige - 1 == y &&
                ...
                if (calculatedPrestige != this.getCurrentPrestige()) {
                    displayedPrestige -=
                        calculatedPrestige - this.getCurrentPrestige();
                }
                bottomText +=
                    '<td><b>E' +
                    displayedPrestige +
    ...
    rewardConfig: {
        ...
        helium: {
            get description() {
                return (
                    heliumOrRadon() +
                    ' gain from all sources is increased by 25%.'
                );
            },
        },
        void: {
            description:
                'Allows up to two Void Maps with the same name to stick together. After completing this 2x Void Map, Fluffy will clear the extra one instantly, granting 50% more Helium than normal and a second Heirloom.',
        },
        eliminator: {
            description:
        ...
        liquid: {
            description:
                'Adds 2.5% to your liquification bonus (half of a Spire).',
        },
        voidance: {
            description:
                'Each Portal, start with two double stacked Void Maps.',
        },
        dailies: {
            get description() {
                return (
                    'Adds 100% to the ' +
                    heliumOrRadon() +
                    ' modifier of all Daily Challenges.'
                );
            },
        },
        critChance: {
        ...
        megaCrit: {
            get description() {
                var text =
                    'Adds +2x to your MegaCrit multiplier, increasing Orange crits to ';
                var current = [
                    getMegaCritDamageMult(2),
                    getMegaCritDamageMult(3),
                ];
                if (!Fluffy.isRewardActive('megaCrit'))
                    current = [current[0] + 2, Math.pow(current[0] + 2, 2)];
                text +=
        ...
        exotic: {
            description:
                'Increases the chance of finding Exotic Imports, bringing the average from 3 per zone to 3.5.',
        },
        reincarnate: {
            description:
                'When a group of Trimps would die, they have a 10% chance to be restored to full health instead. Cannot trigger if Trimps were killed in one hit.',
        },
        heirloopy: {
            description:
...
function mapLevelHotkey(up) {
    ...
    var extraLevelsAvailable =
        getHighestLevelCleared() >= getUnlockZone('extra');
...
document.addEventListener(
    ...
    function(e) {
        ...
        switch (e.keyCode) {
            ...
            case 48: //0
            case 96: //num0
                if (playerSpire.popupOpen) playerSpire.selectTrap('sell');
                break;
            case 49: //1
            case 97: //num1
            ...
            case 88: //x
                if (checkStatus() && game.upgrades.Formations.done)
                    setFormation('0');
                break;
            case 50: //2
            case 98: //num2
            ...
            case 68: //d
                if (checkStatus() && game.upgrades.Dominance.done)
                    setFormation('2');
                break;
            case 52: //4
            case 100: //num4
            ...
            case 66: //b
                if (checkStatus() && game.upgrades.Barrier.done)
                    setFormation('3');
                break;
            case 53: //5
            case 101: //num5
            ...
            case 87: //W
                if (checkStatus() && getUberEmpowerment() == 'Wind')
                    setFormation('5');
            case 55: //7
            case 103: //num7
